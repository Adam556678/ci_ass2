...
class OneMaxGA(GeneticAlgorithm):
    ...
    def standard_decoder(self, chromosome: List[int], minimum: int, maximum: int):
        ...
        chromosome_real_val = minimum + (sum / 2 ** chromosome_length) * (maximum - minimum)
        return math.ceil(chromosome_real_val)

    def evaluate_fitness(self, chromosome: List[int], minimum:int, maximium:int) -> int:
        chromosome_x1_value = self.standard_decoder(chromosome=chromosome[:2], minimum=minimum, maximum=maximium)
        chromosome_x2_value = self.standard_decoder(chromosome=chromosome[2:], minimum=minimum, maximum=maximium)
    ...
    def linear_rank_selection(self, pop: List[List[int]], sp: float):
        ...
        population_fitness = [self.evaluate_fitness(indiv,-2,2) for indiv in pop]
